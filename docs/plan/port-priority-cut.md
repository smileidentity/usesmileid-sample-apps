...
| 8 | **LANDED for both halves.** **Stand up an emulator lane for Flutter and Expo in CI.** **Expo (#113, #114, #115):** `expo/maestro/` plus `.github/workflows/expo-device.yml`. **Flutter:** `flutter/maestro/` — three flows, not a port of the Android suite — plus `.github/workflows/flutter-device.yml`, on ubuntu because `flutter.yml` needs a Mac for host-rasterised goldens and a Mac has no KVM. The flows cover what a hostless Flutter lane structurally cannot see: the bundled licence registry (empty under `flutter test` by design), the cold-start launch-argument plumbing, and #101's three keyguard-blocked behaviours. Expo's numbers did not transfer — 22 of its 28 minutes are native compilation Flutter does not do, and its 60s warm-start tolerance is unnecessary against a measured ~0.7s Flutter cold start. **Five defects found standing it up,** which is what it exists for. Fixed here: a link into a live app served go_router's error page, breaking the mid-run entry `spec/routes.json` documents; the scenario drawer's link was one-shot per session, because dismissing left the router at `/debug/scenarios`; the shell hand-built three of the ten canonical launch arguments, leaving seven — `scenario`, `theme`, `route`, `autostart`, `appLocale`, `holdCamera`, `noticeWindow` — parsed by a function with no caller and ignored on every launch, the same defect class as #101's `seedJobs`; and `sample_license_row`/`_text` did not sanitise their suffix, which `spec/test-ids.json` requires and the other three platforms do. **The nav bar over a partial sheet is CLOSED** (`sheet-nav-bar-layering.md`): the sheet was shown on the tab's navigator, inside the body the bar is painted over, so the pill took the taps meant for the drawer's and the switch sheet's last rows. It now presents on the root navigator as iOS already does, which makes #102's ruling true rather than reversing it — no owner call and no `spec/` change. Two of five sheets were exposed; the pickers escape because nothing above the shell draws a bar, not because they are full height. No baseline moved, because no golden poses a sheet through its presentation helper. `spec/launch-args.json`'s `app_links` claim was also corrected: the app has no such dependency. | Flutter had no device flow at all, and Expo's ran only where someone drove it by hand. Phase 3 waited on a locked handset for behaviours none of which need a camera. Nothing after this item can be called verified without it — and the two defects #113 and #114 fix were both invisible to every hostless lane, as were both of the ones above. | M | review §3.3; Phase 3 recast |
| 9 | **LANDED for Flutter; Expo follows.** **The flow host on Flutter and Expo, as one product's journey end to end, with the job-store contract built inside it.** Product tap → form → SDK flow → result → stored row → list → detail → refresh. Plan 2's four operations, seam and outcome type, plan 3's tolerance fix (substitute, the majority), the wrong-message fallthrough and the never-withdrawn refresh notice are written because this journey calls them, and a cold-start flow in item 8's lane is the definition of done. Restructure Flutter's above-shell routes into a real stack while `/run` is added to the tree. | It is the rest of the port, and the first caller for everything plans 2 and 3 would otherwise build unwired. See review §4.1 and §5.1. **Flutter took SmartSelfie Enrollment, the product the journey helper routes straight to the flow.** Four divergences were recorded rather than aligned: this SDK publishes only two semantics identifiers so the flow asserts the consent screen by text; there is no result card on either port, so the exactly-once terminal-result assertion has no home; a profile here carries no callback URL, so the run submits with the partner's portal default; and the environment is sandbox until a scanned session says otherwise. | L | Phase 4 first half; plans 2, 3 |
| 10 | **Expo shell wiring, restated** (plan 1 without step 3): shell test lane, `seedJobs` at cold start, Dark Mode drives the theme, `noticeWindow`, and — **step 6 LANDED (#107)** — mount the floating nav bar with the clearance from item 7. Add `expo-router/testing-library` in step 1; it is a dev dependency, not a ruling. | Seeding is how the lane gets deterministic rows; the dark-mode switch is a user-visible control that does nothing. All precede any Expo device flow. The nav bar is now mounted and reserves its measured height; the shell test lane, seeding, dark mode and `noticeWindow` remain. | M | findings 6, 7, 10; plan 1 |
...
| 12 | **LANDED (#120).** **Android and iOS: run the app-bar semantics predicate, fix if red.** The three assertions in backlog §7, as a test on each platform. Both were red: Android's back control carried the click action but no `Role.Button` and its title was not a heading; iOS's title was not a header either. Each fix is a line and no baseline moved, which is what the predicate exists to catch. All three assertions run on each platform; iOS reads the header from the snapshot's traits and a merge from element counts, because XCUITest never collapses children — the routes are recorded in backlog §7. | The only Phase 2 item a user can be harmed by (a screen reader cannot reach the title or the back control separately), on shipped apps, and no pixel changes so no baseline can find it. The test is cheap; the fix, if needed, is a line. | S | Phase 2 first item |
...
- `/profiles/switch` and `/profiles/new` are not deep-linkable on Flutter: both match
  `/profiles/:profileId` and render a page for a profile of that name, where Android and iOS open
  the owner with the sheet up. (`sheet-nav-bar-layering.md`)
