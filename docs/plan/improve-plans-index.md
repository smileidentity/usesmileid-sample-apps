...
| 3 | [`improve-port-store-restore-tolerance.md`](improve-port-store-restore-tolerance.md) | One unreadable stored row silently erases **every** verification on Flutter, where Expo drops the row and keeps the rest. Neither port tests the path. Only a write-back (`seedFixtures`, the future `add`) makes the erase permanent; a read alone hides | P1 | S | plan 2's `add`; substitute a default as Android and iOS do, rather than drop as Expo does (`port-priority-cut.md`) | DONE on Flutter — substitutes the first product and Processing, drops only a row with no id; Expo follows in its own tranche |
