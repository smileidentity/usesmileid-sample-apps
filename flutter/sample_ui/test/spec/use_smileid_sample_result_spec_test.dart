...
void main() {
  ...
  test('the route values are the ones the schema enumerates', () {
    final Map<String, Object?> route =
        (schema['properties']! as Map<String, Object?>)['route']!
            as Map<String, Object?>;
    expect(
      UseSmileIDSampleFlowRoute.values.map(
        (UseSmileIDSampleFlowRoute it) => it.id,
      ),
  ...
  test('the environment values are the ones the schema enumerates', () {
    final Map<String, Object?> environment =
        (schema['properties']! as Map<String, Object?>)['environment']!
            as Map<String, Object?>;
    expect(
      UseSmileIDSampleEnvironment.values.map(
        (UseSmileIDSampleEnvironment it) => it.id,
      ),
  ...
  test(
    'a tokenless run reads sandbox, and the host decides from the parsed claim',
    () {
      expect(environmentFor(null), isNull);
      expect(
        environmentFor('https://testapi.smileidentity.com/v3'),
        UseSmileIDSampleEnvironment.sandbox,
      );
      expect(
        environmentFor('https://api.smileidentity.com/v3'),
  ...
  test('cancelled and failed stay separate terminal statuses', () {
    expect(
      UseSmileIDSampleFlowStatus.values.map(
        (UseSmileIDSampleFlowStatus it) => it.id,
      ),
