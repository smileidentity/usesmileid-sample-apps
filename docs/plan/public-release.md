...
**Status:** reviewed 2026-09-25; D1, D7 and D8 approved with #133. T3 done 2026-09-25: every row of
§6 has a destination, and 17 documents are deleted. The rest are deleted as their `docs/` pages land. This doc executes the pre-flip checklist in
...
- **D1 is revised from delete to migrate** (§2). The plans hold the traps that the planned partner
  skills need, so their knowledge moves into partner docs first, and only then
  does the folder go.
...
- **D1 (revised 2026-09-25): migrate `docs/plan/`, then delete it.** The first version deleted it
  ...
  - **Distil.** The durable knowledge (a trap, a measured constraint, a reason a choice was made)
    moves into the partner page from §3 that owns the subject. It is rewritten for a partner,
    not copied.
...
- **D7 (revised 2026-09-25): `docs/plan/` survives, as a public folder.** It holds `backlog.md`, which
  lists every open item, and any plan for work in progress, written for a partner from the first line
  (`docs/plan/README.md`). A plan is deleted by the PR that finishes it. The reasoning a code comment
  needs lives in the `docs/` page that owns the subject, so there is no separate `docs/decisions/`.
...
| **`docs/plan/backlog.md`** | **What is still open** | Done. Every open item from the old plans, written for a public reader (D7). The reasoning a code comment needs goes into the owning page above, e.g. `docs/architecture.md` for the navigation libraries and the iOS 17 floor, `docs/releasing.md` for phone-only and `versionCode` |
...
One row per document. **Distil** names the page that absorbs its durable
...
|---|---|---|---|
| `after-the-ports.md` | carry, drop | — (the sequencing lessons are internal process) | B1–B5, S2 · **deleted 2026-09-25**, open items in `backlog.md` |
| `android-u4-screen-state-goldens.md` | distil, drop | `docs/testing.md`: the state-to-golden table and its exemptions | — |
| `app-store-release-ios.md` | distil, carry | `docs/store-submission.md` (done), `docs/releasing.md`, `docs/releasing.md`: phone-only | A12, A13, A14, O2 |
| `device-suite-cycle-time.md` | carry, distil | `docs/testing.md`: the flakiness guard | A15 · **deleted 2026-09-25**, open items in `backlog.md` |
| `document-capture-options.md` | carry | — | A16 · **deleted 2026-09-25**, open items in `backlog.md` |
...
| `improve-expo-shell-wiring.md` | drop | — | — (done) · **deleted 2026-09-25**, open items in `backlog.md` |
| `improve-expo-structural-checks.md` | carry | — | A5 · **deleted 2026-09-25**, open items in `backlog.md` |
| `improve-flutter-job-store-contract.md` | drop | — | — (done) · **deleted 2026-09-25**, open items in `backlog.md` |
| `improve-plans-index.md` | drop | — | — (A5 carried) · **deleted 2026-09-25**, open items in `backlog.md` |
| `improve-port-store-restore-tolerance.md` | drop | — | — (done) · **deleted 2026-09-25**, open items in `backlog.md` |
...
| `navigation-hardening-android.md` | distil, drop | `docs/architecture.md`: result-back stays out of navigation (NAV-A5), and the rejected features list (NAV-A6) | — |
| `navigation-plan.md` | distil, drop | `docs/architecture.md`: per-platform navigation libraries (`flutter/app/pubspec.yaml` points here), `docs/architecture.md` | — |
| `offline-storage.md` | distil, carry | `docs/architecture.md`: the iOS 17 app floor for SwiftData (`project.yml` and `Package.swift` point here) | A18 |
| `play-release-android.md` | distil, carry | `docs/store-submission.md` (done), `docs/releasing.md`, `docs/releasing.md`: `versionCode` from the commit count | A12, A13, A19, A20, O1, O3, C1 |
| `port-adversarial-review.md` | drop | — | — (its open items live in `port-priority-cut.md`) · **deleted 2026-09-25**, open items in `backlog.md` |
| `port-comment-rationale.md` | drop | — | — · **deleted 2026-09-25**, open items in `backlog.md` |
| `port-gaps-backlog.md` | carry | — | A1–A4, A6, B1–B2 · **deleted 2026-09-25**, open items in `backlog.md` |
| `port-patterns.md` | distil, drop | `docs/architecture.md` (`scripts/test_sync_design_tokens.py` points here) | — |
| `port-priority-cut.md` | carry | — | A7–A9, B4. Item 6 (fail-closed gates) is P9 and O4 · **deleted 2026-09-25**, open items in `backlog.md` |
| `port-review-findings.md` | carry | — | A10, A11 · **deleted 2026-09-25**, open items in `backlog.md` |
| `ports-final.md` | carry | — | C2 · **deleted 2026-09-25**, open items in `backlog.md` |
...
| `ui-feedback-pass.md` | drop | — | — (F50 and F51 carried as A22 and A23) · **deleted 2026-09-25**, open items in `backlog.md` |
...
**Destinations.** **A** goes into `docs/plan/backlog.md`, written for a public reader (ruled
2026-09-25: one backlog file rather than one issue per item). **B** is a product ruling, which goes to the team's private tracker and is never
...
### A. Engineering backlog: `docs/plan/backlog.md`

| # | Item | Source | State |
|---|---|---|---|
| A1 | "Tap \`Hide from List\` to confirm" renders its backticks literally on Android and iOS; baselines move | `port-gaps-backlog.md` §2 | **verified open** (`UseSmileIDSampleSelectionBar.kt:79`, `.swift:88`) · ✅ carried to `backlog.md`: *Literal backticks in the selection bar hint on Android and iOS* |
| A2 | The Android job row's secondary line should take the board's caption; row height moves | `port-gaps-backlog.md` §2 | unverified · ✅ carried to `backlog.md`: *Android: job row caption and a stale status-badge comment* |
| A3 | Android's `UseSmileIDSampleStatusBadge` doc comment is stale about the soft badge pairs | `port-gaps-backlog.md` §2 | unverified · ✅ carried to `backlog.md`: *Android: job row caption and a stale status-badge comment* |
| A4 | Id spec tests should assert both directions on Android and Flutter, and Android and Expo need the id-attached check that iOS (`TestIdUsageTest`) and Flutter have | `port-gaps-backlog.md` §2, `port-priority-cut.md` §2 | unverified · ✅ carried to `backlog.md`: *Test-id and semantics checks are missing on Android and Expo* |
| A5 | Expo lacks the two structural checks (declared ids attached to something, containers not absorbing children's semantics), and its app-bar title has no header role | `improve-expo-structural-checks.md` (TODO in its index) | unverified · ✅ carried to `backlog.md`: *Test-id and semantics checks are missing on Android and Expo* |
| A6 | iOS picks ink by perceptual grey (`UIColor.getWhite`), where the others use WCAG luminance; 12 of 51 colours differ. Check the 12 against WCAG AA, and fix the function if any fails | `port-gaps-backlog.md` §2 | **verified open** (`UseSmileIDSampleColorMath.swift:8`) · ✅ carried to `backlog.md`: *iOS: pick text colour by WCAG luminance, as the other three apps do* |
| A7 | One owner table in `spec/README.md` for spec debt: the two shell ids no app implements, `sample_env_chip`, `sample_license_link`, the licences wording, button 48 vs 52, glyph 21 vs 20, `screens.json`'s internal contradiction, and whether `components.json` is informative or checked | `port-priority-cut.md` §2 | unverified · ✅ carried to `backlog.md`: *One table for spec debt in `spec/README.md`* |
| A8 | A dead-export check: `knip` on the Expo workspace, unused-code rules on Flutter | `port-priority-cut.md` §2 | unverified · ✅ carried to `backlog.md`: *A dead-code check for Expo and Flutter* |
| A9 | An envelope-pumping helper for layout predicates: narrowest width, largest scale, real font, non-zero inset | `port-priority-cut.md` §2 | unverified · ✅ carried to `backlog.md`: *Layout predicates that can fail* |
| A10 | Flutter's text-scale predicate cannot fail on truncation, because `didExceedMaxLines && maxLines != 1` excludes every paragraph able to report | `port-review-findings.md` #8 | unverified · ✅ carried to `backlog.md`: *Layout predicates that can fail* |
| A11 | The Flutter nav-bar clearance test sweeps five text scales against one tab root | `port-review-findings.md` #12 | unverified · ✅ carried to `backlog.md`: *Layout predicates that can fail* |
| A12 | The camera store panel on both stores needs a device run (the flow and output path exist) | `play-release-android.md` REL-A9, `app-store-release-ios.md` §7.2 | unverified · ✅ carried to `backlog.md`: *Store art: camera panel, a sparse details panel, shared demo data* |
| A13 | The `verification_details` store panel is sparse without the debug result card: ship it, drop to four panels, or swap in a denser state | both store plans | open by choice · ✅ carried to `backlog.md`: *Store art: camera panel, a sparse details panel, shared demo data* |
| A14 | Lift the store-art demo literals (organisation, initials, session id, countdown) into `spec/`, so the two stores' art cannot drift | `app-store-release-ios.md` §7.2 | unverified · ✅ carried to `backlog.md`: *Store art: camera panel, a sparse details panel, shared demo data* |
| A15 | Device-suite cycle time: relevance-based flow selection, debug-only per PR with release on `main` and on packaging changes, sharding, and requiring AC power for a full run | `device-suite-cycle-time.md` (proposed, nothing built) | unverified · ✅ carried to `backlog.md`: *Make the device suite fast enough to run on every PR* |
| A16 | Document-capture options the sample does not exercise: direct document-type selection, `captureBothSides` following the type, capture mode as a setting asserted on the wire, gallery upload, known aspect ratio (DOC-A0 to A8) | `document-capture-options.md` | unverified · ✅ carried to `backlog.md`: *Exercise the SDK's document-capture options* |
| A17 | iOS device lane: the camera permission prompt is the one §2.3 bullet left | `ios-device-verification.md` §2.3 | unverified · ✅ carried to `backlog.md`: *iOS device lane: handle the camera permission prompt* |
| A18 | Android's job-store undo silently discards when asked to remove an id it has no row for; iOS guards on what the removal took | `offline-storage.md` | unverified · ✅ carried to `backlog.md`: *Android: removing an unknown job id discards the undo* |
| A19 | Make the Play internal lane run on `push: main`, with a path filter so a docs-only merge does not publish | `play-release-android.md` §7.4 | unverified · ✅ carried to `backlog.md`: *Play publishing: an internal build on every merge, and a collision-proof versionCode* |
| A20 | Derive `versionCode` from Play's highest code + 1, as iOS now does with `asc_publish.py next-build`, to end the same-ref double-dispatch collision (weighed and deferred once) | `play-release-android.md` §7.4 | open by choice · ✅ carried to `backlog.md`: *Play publishing: an internal build on every merge, and a collision-proof versionCode* |
| A21 | The registry-consumption lane in this repo: a build-id surface, clean install, resolve → release build → launch → drive, a snapshot arm, the job-summary contract, a companion arm, triage doc, and ports (CON-A1 to A9). CON-A7 needs the SDK repos' agreement | `registry-consumption-e2e.md` | unverified · ✅ carried to `backlog.md`: *A registry-consumption lane that launches and drives the published SDK* |
| A22 | Every sheet route replaces the screen beneath it on Android, so the scrim covers a bare window. The recommendation is an overlay presentation, adopted with Navigation 3 | `sample-apps-plan.md` §8.2, `ui-feedback-pass.md` F50 | unverified · ✅ carried to `backlog.md`: *Android: sheets cover a blank window instead of the screen beneath* |
| A23 | Pull-to-refresh on the verifications list: unblocked (a refresh matches the partner, not the session), with the shape §8.1 lists | `sample-apps-plan.md` §8.1, `ui-feedback-pass.md` F51 | unverified. `spec/screens.json` says Android replaced the button; check each platform · ✅ carried to `backlog.md`: *Pull to refresh on the verifications list* |
| A24 | The Flutter iOS app needs `NSLocationWhenInUseUsageDescription` and `NSPhotoLibraryUsageDescription` before its first upload. `nm -u` the Expo iOS release build too | P20 | **verified open** · fixed by its own PR (step 7), not carried |
| A25 | Android `sample-ui` declares no `resourcePrefix`, so its `sample_ic_*` prefix is hand-kept and unenforced | `sample-apps-plan.md`, its removed resume block | **verified open** · ✅ carried to `backlog.md`: *Android: enforce a resource prefix on `sample-ui`* |

### B. Product rulings: the team's private tracker

| # | Question | Source | State |
|---|---|---|---|
| B1 | Should a profile's stored defaults seed the job form, and should "remember these details" remember anything? | `port-gaps-backlog.md` §4, `after-the-ports.md` Phase 1 | unverified since `profiles-ux.md` (DONE 2026-09-25) · ✅ in the private tracker, 2026-09-25 |
| B2 | The active-profile edit | same | **appears closed**: iOS reads "Save changes" when the profile is active (`ProfileConfigScreen.swift:109`). Confirm on all four, then close · closed, not carried |
| B3 | Button height 48 vs 52, and card glyph 21 vs 20: the spec and the design disagree | `after-the-ports.md` Phase 1 | unverified · ✅ in the private tracker, 2026-09-25 |
| B4 | The two shell ids in `spec/test-ids.json` that no app implements: build them or delete them | `after-the-ports.md` Phase 1, `ports-final.md` §3 | unverified · ✅ in the private tracker, 2026-09-25 |
| B5 | May `expo/app` declare `expo-router/testing-library` (test-only) to assert a cold deep link's navigation state? | `port-gaps-backlog.md` §4 | unverified · ✅ in the private tracker, 2026-09-25 |
