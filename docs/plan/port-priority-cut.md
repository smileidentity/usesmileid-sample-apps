...
| 8 | **LANDED for both halves.** **Stand up an emulator lane for Flutter and Expo in CI.** **Expo (#113, #114, #115):** `expo/maestro/` plus `.github/workflows/expo-device.yml`. **Flutter:** `flutter/maestro/` — three flows, not a port of the Android suite — plus `.github/workflows/flutter-device.yml`, on ubuntu because `flutter.yml` needs a Mac for host-rasterised goldens and a Mac has no KVM. The flows cover what a hostless Flutter lane structurally cannot see: the bundled licence registry (empty under `flutter test` by design), the cold-start launch-argument plumbing, and #101's three keyguard-blocked behaviours. Expo's numbers did not transfer — 22 of its 28 minutes are native compilation Flutter does not do, and its 60s warm-start tolerance is unnecessary against a measured ~0.7s Flutter cold start. **Two defects found on the lane's first run,** which is what it exists for: a link into a live app served go_router's error page, breaking the mid-run entry `spec/routes.json` documents (fixed here, with the `app_links` claim in `spec/launch-args.json` corrected — the app has no such dependency); and the floating nav bar is drawn over the bottom of every partial sheet, so the scenario drawer's last two theme rows cannot be tapped (**open** — the fix is in `sample_ui`'s shared sheet, moves three sheets' baselines, and Expo solved the same class the opposite way in #114, so which way Flutter goes is a four-platform call). | Flutter had no device flow at all, and Expo's ran only where someone drove it by hand. Phase 3 waited on a locked handset for behaviours none of which need a camera. Nothing after this item can be called verified without it — and the two defects #113 and #114 fix were both invisible to every hostless lane, as were both of the ones above. | M | review §3.3; Phase 3 recast |
