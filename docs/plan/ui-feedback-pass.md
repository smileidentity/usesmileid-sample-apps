...
### Owed, to review later — F71 closed 2026-08-18

| ID | Item | Status |
|---|---|---|
| F71 | The nav bar's absence on a pushed screen was asserted on the **deep-link** arrival only | done — `verifications.yaml` now asserts it on the tap path too, where it reaches the details screen through `sample_job_row_0`. The property belongs to the destination, so a regression can no longer hide behind whichever arrival a flow does not exercise |
