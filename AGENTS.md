...
- **Agents: open and update PRs through the `create-pr` skill, never by hand.** It runs the
  pre-PR review, fills the description from what the author actually said rather than from the diff,
  and keeps internal context out of what reviewers read. Hand-rolling `gh pr create` skips all three.
