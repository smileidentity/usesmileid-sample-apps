...
| 10 | The Expo shell shipped the stock tab bar and never mounted the designed floating nav bar, which sat in `sample-ui` with zero call sites | closed by #107: mounted as the Tabs `tabBar`, labels driven from `smileIDSampleNavItems`, and the three tab screens now reserve the bar's MEASURED height rather than nothing. Measuring it first showed `Verifications` already breaking mid-word at 320pt at default text size |
