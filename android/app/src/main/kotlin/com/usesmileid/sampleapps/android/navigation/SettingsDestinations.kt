...
fun SettingsScreen(navigator: DestinationsNavigator) {
    ...
    SettingsContent(
        ...
        // because Apache-2.0 §4 asks the notice to travel with the distribution rather than be linked.
        onNavRowClick = { row ->
...
/**
 * The notices screen. Its data is generated at build time into `sample-ui`'s assets, so it is read
 * once here and handed in — the screen stays a function of its arguments like every other.
 */
...
fun LicensesScreen(navigator: DestinationsNavigator) {
    val context = LocalContext.current
    val chrome = LocalUseSmileIDSampleChrome.current
    // Off the main thread: the asset is the whole Apache-2.0 text plus two hundred entries.
...
// Generated by scripts/generate_licenses.py into sample-ui, which is the module that renders it.
